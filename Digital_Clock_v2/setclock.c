...
/**
 *	该函数用于把 MCLK,SMCLK倍频到 24MHZ ，ACLK不变化是32.768KHz
 *
 *
 **/
void SetClock_MCLK12MHZ_SMCLK12MHZ_ACLK32_768K(void)
{
    ...
    UCSCTL0 = 0x0000;                         // Set lowest possible DCOx, MODx     // DCO = 0, MOD = 0
    UCSCTL1 = DCORSEL_7;                      // Select DCO range 24MHz operation
    UCSCTL2 = FLLD_1 + 374;                   // Set DCO Multiplier for 12MHz       // N=776
                                              // (N + 1) * FLLRef = Fdco
                                              // (374 + 1) * 32768 = 12MHz
